...
def compute_sh_free(
    ...
    return _ComputeSHFree.apply(shs, degree, view_dirs, visible)


class _ComputeSHFree(torch.autograd.Function):
    @staticmethod
    def forward(ctx, shs, degree, view_dirs, visible):
        color = _C.compute_sh_free_forward(shs, degree, view_dirs, visible)
    ...
    def backward(ctx, dL_dcolor):
        ...
        (dL_dshs, dL_dvdirs) = _C.compute_sh_free_backward(
            shs, degree, view_dirs, visible, dL_dcolor
        )
