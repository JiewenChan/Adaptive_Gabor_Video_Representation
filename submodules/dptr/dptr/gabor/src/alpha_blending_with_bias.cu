...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
    alphaBlendingForwardCUDAKernel(const int P,
                                   ...
                                   const float *__restrict__ opacity_bias,
    ...
    __shared__ float collected_opacity_bias[BLOCK_SIZE];
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
        ...
        if (range.x + progress < range.y) {
            ...
            collected_opacity_bias[block.thread_rank()] = opacity_bias[coll_id];
        }
        block.sync();
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
    alphaBlendingBackwardCUDAKernel(const int P,
                                    ...
                                    float *__restrict__ dL_dopacity_bias) {
    auto block = cg::this_thread_block();
    int32_t tile_grid_x = (W + BLOCK_X - 1) / BLOCK_X;
    int32_t tile_id =
        block.group_index().y * tile_grid_x + block.group_index().x;
    ...
    for (int C0 = 0; C0 < C;) {
        ...
        if (C - C0 <= 3) {
            alphaBlendingForwardCUDAKernel<3><<<tile_grid, block>>>(
                ...
                opacity_bias.contiguous().data_ptr<float>(),
...
alphaBlendingBackwardWithBias(const torch::Tensor &uv,
    ...
    torch::Tensor dL_dopacity_bias = torch::zeros({P, 1}, float_opts);
    ...
    // [C, N]
    torch::Tensor feature_permute = feature.transpose(0, 1);
    torch::Tensor opacity_bias_permute = opacity_bias.transpose(0, 1);
    ...
    for (int C0 = 0; C0 < C;) {
        ...
        if (C - C0 <= 3) {
            alphaBlendingBackwardCUDAKernel<3><<<tile_grid, block>>>(
                ...
                opacity.contiguous().data_ptr<float>(),
                feature_permute.contiguous().data_ptr<float>() + p_data_offset,
                opacity_bias_permute.contiguous().data_ptr<float>(),
                idx_sorted.contiguous().data_ptr<int>(),
                (int2 *)tile_range.contiguous().data_ptr<int>(),
                bg,
                C - C0,
                ...
                dL_dopacity_bias.data_ptr<float>());
            C0 += 3;
        } else if (C - C0 <= 6) {
    ...
    return std::make_tuple(dL_duv, dL_dconic, dL_dopacity, dL_dfeature, dL_dopacity_bias, dL_dabs_uv);
}