...
class RAFTExhaustiveDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        flow_normed = flow / np.array([self.w, self.h])[None,None] * 2
        data = {'ids1': id1,
                'ids2': id2,
                'pts1': pts1,  # [n_pts, 2]
                'pts2': pts2,  # [n_pts, 2]
                ...
                'gt_img' : torch.from_numpy(img1).float().permute(2, 0, 1), # [3, H, W]
                'gt_flow' : torch.from_numpy(flow_normed).float().permute(2, 0, 1), # [2, H, W]
                }
        return data