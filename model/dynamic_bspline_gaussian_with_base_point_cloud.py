...
class DynamicBsplineGaussianWithBasePointCloud(PointCloud):
    """
    ...
    def setup(self, gs_atlas_cfg, base_point_seq=None):
        ...
        self.register_atribute("pos_cubic_node", self.cubic_coeff.reshape(-1, self.interval_num*3))
    ...
    def get_position(self, time, detach_pos=False):
        ...
        if isinstance(time, torch.Tensor):
            time = time.item()
        normed_time = time / (len(self.delta_position))
