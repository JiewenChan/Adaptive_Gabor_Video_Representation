...
class AbsGSOptimizer(BaseOptimizer):
    ...
    class Config:
        ...
        vis_every_n: int = 200
        vis_out_dir: str = "output/vis/densify"
        vis_local_knn: int = 0
        vis_max_points: int = 100000
        vis_use_importance: bool = True
        vis_export_ply: bool = True
        vis_wave_coefficients: bool = True
        vis_wave_coeff_every_n: int = 200
        ...
        vis_dynamic: bool = True
        vis_dynamic_frames: int = 20
    ...
    def update_structure(self, visibility: Tensor, viewspace_grad: Tensor,
        ...
        if self.step < self.cfg.densify_stop_iter:
            ...
            if self.opacity_deferred:
                self.opacity_deferred = False
                self.reset_opacity()
                self.last_reset_opacity_step = self.step
    ...
    def update_model(self, viewspace_points: Tensor, viewspace_points_grad: Tensor,
        ...
        viewspace_points_grad: torch.Tensor
            The gradient of the view space points.
        visibility : torch.Tensor
            The visibility of the points.
        radii : torch.Tensor
            The radii of the point cloud.
    ...
    @torch.no_grad()
    def accumulate_viewspace_grad(self, viewspace_points: Tensor, viewspace_points_grad: Tensor) -> Tensor:
        """
        ...
        for vp, vp_grad in zip(viewspace_points, viewspace_points_grad):
            viewspace_grad += vp_grad
